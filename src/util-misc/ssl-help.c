...
char *ipaddr_to_asc(unsigned char *p, int len) {
    ...
    switch (len) {
        case 4: /* IPv4 */
            BIO_snprintf(buf, sizeof(buf), "%u.%u.%u.%u", p[0], p[1], p[2],
                         p[3]);
            break;
        ...
        case 16: /* IPv6 */
            for (out = buf, i = 8, remain = sizeof(buf); i-- > 0 && bytes >= 0;
                ...
                bytes = (ptrdiff_t)BIO_snprintf(out, remain, template,
                                                p[0] << 8 | p[1]);
                p += 2;
            }
            break;
...
void init_openssl_ext_obj() {
    ...
    nid_DomainController = add_ext_obj(
        OID_DomainController, SN_DomainController, LN_DomainController);
            ...
            switch (nid) {
                ...
                default:
                    if (nid == NID_undef) {
                        char sz_obj[256];
                        int  len_sz_obj;
                        len_sz_obj = OBJ_obj2txt(sz_obj, sizeof(sz_obj),
                                                 gen->d.otherName->type_id, 0);
                        LOG(LEVEL_WARN,
                            "(GENERAL_NAME_simple_print) unknown GEN_OTHERNAME "
                            "oid "
                            "%d(%.*s)\n",
                    ...
                    } else if (nid == nid_SRVName ||
                               nid == nid_AttributeSyntaxVendor) {
                        ret = ASN1_STRING_print_ex(
                            out, gen->d.otherName->value->value.ia5string, 0);
                        if (ret < 0) {
                            ret = -1;
                        } else {
                            ret = 1;
                        }
                    } else if (nid == nid_KRB5PrincipalName ||
                               nid == nid_DomainController ||
                               nid == nid_CiscoDlswTConnOperTable ||
                               nid == NID_description ||
