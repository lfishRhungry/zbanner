...
static size_t lzr_k8s_make_payload(ProbeTarget   *target,
                                   unsigned char *payload_buf) {
    memcpy(payload_buf, lzr_k8s_payload, sizeof(lzr_k8s_payload) - 1);
    return sizeof(lzr_k8s_payload) - 1;
}

static size_t lzr_k8s_get_payload_length(ProbeTarget *target) {
    return sizeof(lzr_k8s_payload) - 1;
}

static unsigned lzr_k8s_handle_reponse(unsigned th_idx, ProbeTarget *target,
...
Probe LzrK8sProbe = {
    ...
    .multi_num  = 1,
    .params     = NULL,
    .desc = "LzrK8s Probe sends a kubernetes probe and identifies kubernetes "
            "service.",
