...
static size_t lzr_siemens_make_payload(ProbeTarget   *target,
                                       unsigned char *payload_buf) {
    memcpy(payload_buf, lzr_siemens_payload, sizeof(lzr_siemens_payload) - 1);
    return sizeof(lzr_siemens_payload) - 1;
}

static size_t lzr_siemens_get_payload_length(ProbeTarget *target) {
    return sizeof(lzr_siemens_payload) - 1;
}

static unsigned lzr_siemens_handle_reponse(unsigned th_idx, ProbeTarget *target,
...
Probe LzrSiemensProbe = {
    ...
    .multi_num  = 1,
    .params     = NULL,
    .desc = "LzrSiemens Probe sends an Siemens probe and identifies Siemens "
            "service.",
