...
static char lzr_socks5_payload[] =
    "\x05\x04\x00\x01\x02\x80\x05\x01\x00\x03\x0agoogle.com\x00\x50GET / "
    "HTTP/1.0\r\n\r\n";

static size_t lzr_socks5_make_payload(ProbeTarget   *target,
                                      unsigned char *payload_buf) {
    memcpy(payload_buf, lzr_socks5_payload, sizeof(lzr_socks5_payload) - 1);
    return sizeof(lzr_socks5_payload) - 1;
}

static size_t lzr_socks5_get_payload_length(ProbeTarget *target) {
    return sizeof(lzr_socks5_payload) - 1;
}

static unsigned lzr_socks5_handle_reponse(unsigned th_idx, ProbeTarget *target,
...
static unsigned lzr_socks5_handle_timeout(ProbeTarget *target, OutItem *item) {
    item->level = OUT_FAILURE;
    safe_strcpy(item->classification, OUT_CLS_SIZE, "not socks5");
