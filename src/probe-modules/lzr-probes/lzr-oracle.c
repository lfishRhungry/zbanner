    ...
    struct OutputItem *item)
{
    ...
    if (safe_memmem(px, sizeof_px, "DESCRIPTION=(", strlen("DESCRIPTION=("))
        ...
        safe_strcpy(item->classification, OUTPUT_CLS_SIZE, "oracle");
        safe_strcpy(item->reason, OUTPUT_RSN_SIZE, "matched");
        return 0;
    }

    item->level = Output_FAILURE;
    safe_strcpy(item->classification, OUTPUT_CLS_SIZE, "not oracle");
...
static unsigned
lzr_oracle_handle_timeout(struct ProbeTarget *target, struct OutputItem *item)
{
    item->level = Output_FAILURE;
    safe_strcpy(item->classification, OUTPUT_CLS_SIZE, "not oracle");
