...
static size_t lzr_amqp_make_payload(ProbeTarget   *target,
                                    unsigned char *payload_buf) {
    memcpy(payload_buf, lzr_amqp_payload, sizeof(lzr_amqp_payload) - 1);
    return sizeof(lzr_amqp_payload) - 1;
}

static size_t lzr_amqp_get_payload_length(ProbeTarget *target) {
    return sizeof(lzr_amqp_payload) - 1;
}

static unsigned lzr_amqp_handle_reponse(unsigned th_idx, ProbeTarget *target,
...
static unsigned lzr_amqp_handle_timeout(ProbeTarget *target, OutItem *item) {
    item->level = OUT_FAILURE;
    safe_strcpy(item->classification, OUT_CLS_SIZE, "not amqp");
...
Probe LzrAmqpProbe = {
    ...
    .multi_num  = 1,
    .params     = NULL,
    .desc = "LzrAmqp Probe sends an AMQP probe and identifies AMQP service.",
