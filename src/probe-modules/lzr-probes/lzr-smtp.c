...
static char lzr_smtp_payload[] = "\x45\x48\x4c\x4f\x0d\x0a";

static size_t lzr_smtp_make_payload(ProbeTarget   *target,
                                    unsigned char *payload_buf) {
    memcpy(payload_buf, lzr_smtp_payload, sizeof(lzr_smtp_payload) - 1);
    return sizeof(lzr_smtp_payload) - 1;
}

static size_t lzr_smtp_get_payload_length(ProbeTarget *target) {
    return sizeof(lzr_smtp_payload) - 1;
}

static unsigned lzr_smtp_handle_reponse(unsigned th_idx, ProbeTarget *target,
...
Probe LzrSmtpProbe = {
    ...
    .multi_num  = 1,
    .params     = NULL,
    .desc = "LzrSmtp Probe sends an SMTP probe and identifies SMTP service.",
