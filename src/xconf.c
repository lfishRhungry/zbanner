...
/***************************************************************************
 ***************************************************************************/
typedef int (*SET_PARAMETER)(struct Xconf *xconf, const char *name, const char *value);
     ...
     */
    {
        ...
        for (i=0; config_parameters[i].name; i++) {
            if (EQUALS(config_parameters[i].name, name)) {
                if (CONF_ERR == config_parameters[i].set(xconf, name, value))
                    exit(0);
                return;
            } else {
                size_t j;
...
{
    for (size_t i=0; config_parameters[i].name; i++) {
        if (EQUALS(config_parameters[i].name, name)) {
    ...
    for (i=1; i<argc; i++) {
        ...
        if (argv[i][0] == '-') {
            ...
            if (is_numable(argname)) {
                ...
                xconf_set_parameter(xconf, name2, value);
            } else {
                value = strchr(&argv[i][2], '=');
                if (value == NULL)
                    value = strchr(&argv[i][2], ':');
