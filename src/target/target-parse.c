...
/**
 * Given an address 'being' and a 'prefix', return the 'begin' and 'end'
        ...
        switch (state) {
            ...
            case IPV6_CIDR_NUM:
                switch (c) {
                    ...
                    case '\r':
                    case '\n': {
                        _ipv6_apply_cidr(&p->ipv6._begin, &p->ipv6._end,
                                         p->tmp);
            ...
            case SECOND3:
                switch (c) {
                    ...
                    case 0xe2:
                        ...
                        break;
                    case '-':
                    case 0x96: /* long dash, comes from copy/pasting into
                                * exclude files
                                */
                        if (state == NUMBER3) {
                    ...
                    case ':':
                        if (state == NUMBER0) {
                            /* Assume this is an IPv6 address instead of an IPv4
                             * address
                             */
                            _switch_to_ipv6(p, state);
                            state = IPV6_BEGIN;
...
int targetset_parse_file(TargetSet *targetset, const char *filename) {
    ...
    while (!is_error) {
        ...
        while (offset < count) {
            ...
            switch (err) {
                case Still_Working:
                    if (offset < count) {
                        /* We reached this somehow in the middle of the buffer,
                         * but this return is only possible at the end of the
                         * buffer */
                        LOG(LEVEL_ERROR,
                            "rangeparse_next(): unknown coding failure\n");
                    }
                    break;
                case Found_Error:
                default:
    ...
    LOG(LEVEL_DEBUG, "%s: %u addresses read\n", filename, addr_count);
        ...
        case Still_Working:
            if (offset < count) {
                ...
                if (err == Still_Working) {
                    LOG(LEVEL_ERROR,
                        "_parser_next(): unknown coding failure\n");
                    goto fail;
                } else {
                    goto again;
                }
