...
bool dns_parse_record(uint8_t *begin, uint8_t *buf, const uint8_t *end, uint8_t **next, dns_record_t *record)
{
    ...
    if (record->type == DNS_REC_A)
    {
        ...
        record->data.addr.ipv4 = BE_TO_U32(record->data.raw);
        record->data.addr.version = 4;
    }
    else if (record->type == DNS_REC_AAAA)
    {
        ...
        record->data.addr.ipv6 = ipv6address_from_bytes(record->data.raw);
        record->data.addr.version = 6;
    }
...
char* dns_raw_record_data2str(dns_record_t *record, uint8_t *begin, uint8_t *end, bool put_quotes)
{
    ...
    switch(record->type)
    {
        ...
        case DNS_REC_A:
            ...
            ipv4_byte2str(record->data.raw, buf, ARRAY_SIZE(buf));
            break;
        case DNS_REC_AAAA:
            if(record->length != 16)
            {
            ...
            ipv6_byte2str(record->data.raw, buf, ARRAY_SIZE(buf));
            break;
        case DNS_REC_CAA:
