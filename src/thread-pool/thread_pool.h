...
thread_pool_t *
createThreadPool(unsigned int fix_thread_num, unsigned int max_thread_num);

void
destroyThreadPool(thread_pool_t *pool);


BOOL
addJobToThreadPool(thread_pool_t *pool, USER_FUNC process_job, void *args);

BOOL
addJobToThreadPoolEx(thread_pool_t *pool,
    USER_FUNC process_job,
    USER_FUNC release_job,
    void *args, time_out_t *time_out);

void
tp_sleep(unsigned int ms);
