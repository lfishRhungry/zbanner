...
/**
 * Nmap probe format is a half-hex like and allows escaped string. It is convenient
 * for record of probe data and tells us what to send.
 * 
 ...
 * 
 * NOTE: Originally, it must start with a q, then a delimiter character which begins 
 * and ends the string. Between the delimiter characters is the string that is 
 * actually sent. But the `q` and delimiters are not necessary here.
*/
 ...
 */
size_t nmapprobe_decode(const char *str, size_t slen, void *buf, size_t bufsize);
